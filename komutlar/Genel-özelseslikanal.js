...
exports.run = async (client, message, args) => {
    ...
    const privateChannelID = '836608704370573353'; // ! Değişecek
    const categoryID = '836608115817709609'; // ! Değişecek

    if (message.channel.id != privateChannelID) return error(`Bu komut <#${privateChannelID}> kanalında kullanılabilir.`);
    const option = args[0];
    const options = ['oluştur','izin','sil'];
    try {
        switch (option) {
            case 'oluştur':
                ...
                message.guild.channels.create(channelName, {
                    ...
                    const interval = setInterval(() => {
                        if (createdChannel.members.size == 0) {
                            ...
                            message.channel.send(``, new Discord.MessageEmbed().setColor('GREEN').setTitle('Silme başarılı!').setDescription(`**${message.author} özel kanalınız hareketsizlik sebebiyle silindi.**`));
              ...
              break;
            case 'izin':
                if (!database.has(`privateChannels.${message.author.id}`)) return error('Zaten herhangi bir özel kanala sahip değilsiniz ve ya odanın sahibi değilsiniz.');
                ...
                if (message.mentions.members.size <= 1) {
                    ...
                    if (privateChannel1.permissionsFor(user).has('CONNECT')) {
                        privateChannel1.updateOverwrite(user, {
                            ...
                            message.channel.send(new Discord.MessageEmbed().setColor('GREEN').setTitle('İzin silindi!').setDescription(`**${user} kişisinin, özel kanalınıza girme yetkisi kaldırıldı.**`));
                        });
                    } else {
                        privateChannel1.updateOverwrite(user, {
                            CONNECT: true
                ...
                break;
            default:
                error(`Lütfen komutu doğru kullanın!\n\n\`• st!özelkanal oluştur <üyeSayısı> <kanalAdı>\`\n\`• st!özelkanal izin @üye\``);
        };
    } catch (err) {
